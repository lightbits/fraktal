...
{
    if (!parse_char(c, '('))  { parse_error(*c, "Error parsing integer tuple: must begin with parenthesis.\n"); return false; }
    if (!parse_int(c, &v->x)) { parse_error(*c, "Error parsing integer tuple: first component must be an integer.\n"); return false; }
    if (!parse_char(c, ','))  { parse_error(*c, "Error parsing integer tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_int(c, &v->y)) { parse_error(*c, "Error parsing integer tuple: second component must be an integer.\n"); return false; }
    if (!parse_char(c, ')'))  { parse_error(*c, "Error parsing integer tuple: must end with parenthesis.\n"); return false; }
    return true;
}
...
static bool parse_int3(const char **c, int3 *v)
{
    ...
    if (!parse_int(c, &v->y)) { parse_error(*c, "Error parsing integer tuple: second component must be an integer.\n"); return false; }
    if (!parse_char(c, ','))  { parse_error(*c, "Error parsing integer tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_int(c, &v->z)) { parse_error(*c, "Error parsing integer tuple: second component must be an integer.\n"); return false; }
    if (!parse_char(c, ')'))  { parse_error(*c, "Error parsing integer tuple: must end with parenthesis.\n"); return false; }
    return true;
}
...
static bool parse_int4(const char **c, int4 *v)
{
    ...
    if (!parse_int(c, &v->z)) { parse_error(*c, "Error parsing integer tuple: second component must be an integer.\n"); return false; }
    if (!parse_char(c, ','))  { parse_error(*c, "Error parsing integer tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_int(c, &v->w)) { parse_error(*c, "Error parsing integer tuple: second component must be an integer.\n"); return false; }
    if (!parse_char(c, ')'))  { parse_error(*c, "Error parsing integer tuple: must end with parenthesis.\n"); return false; }
    return true;
}
...
{
    if (!parse_char(c, '('))        { parse_error(*c, "Error parsing angle tuple: must begin with parenthesis.\n"); return false; }
    if (!parse_angle(c, &v->theta)) { parse_error(*c, "Error parsing angle tuple: first component must be a number.\n"); return false; }
    if (!parse_char(c, ','))        { parse_error(*c, "Error parsing angle tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_angle(c, &v->phi))   { parse_error(*c, "Error parsing angle tuple: second component must be a number.\n"); return false; }
    if (!parse_char(c, ')'))        { parse_error(*c, "Error parsing angle tuple: must end with parenthesis.\n"); return false; }
    return true;
}

static bool parse_float2(const char **c, float2 *v)
{
    if (!parse_char(c, '('))    { parse_error(*c, "Error parsing tuple: must begin with parenthesis.\n"); return false; }
    if (!parse_float(c, &v->x)) { parse_error(*c, "Error parsing tuple: first component must be a number.\n"); return false; }
    if (!parse_char(c, ','))    { parse_error(*c, "Error parsing tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_float(c, &v->y)) { parse_error(*c, "Error parsing tuple: second component must be a number.\n"); return false; }
    if (!parse_char(c, ')'))    { parse_error(*c, "Error parsing tuple: must end with parenthesis.\n"); return false; }
    return true;
}
...
static bool parse_float3(const char **c, float3 *v)
{
    ...
    if (!parse_float(c, &v->y)) { parse_error(*c, "Error parsing tuple: second component must be a number.\n"); return false; }
    if (!parse_char(c, ','))   { parse_error(*c, "Error parsing tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_float(c, &v->z)) { parse_error(*c, "Error parsing tuple: third component must be a number.\n"); return false; }
    if (!parse_char(c, ')'))   { parse_error(*c, "Error parsing tuple: must end with parenthesis.\n"); return false; }
    return true;
}
...
static bool parse_float4(const char **c, float4 *v)
{
    ...
    if (!parse_float(c, &v->z)) { parse_error(*c, "Error parsing tuple: third component must be a number.\n"); return false; }
    if (!parse_char(c, ','))   { parse_error(*c, "Error parsing tuple: components must be seperated by ','.\n"); return false; }
    if (!parse_float(c, &v->w)) { parse_error(*c, "Error parsing tuple: third component must be a number.\n"); return false; }
    if (!parse_char(c, ')'))   { parse_error(*c, "Error parsing tuple: must end with parenthesis.\n"); return false; }
    return true;
}
...
static bool parse_next_in_list(const char **c)
{
    ...
    if (parse_char(c, ')'))
        return false;
    if (!parse_list_first)
        if (!parse_char(c, ',')) { parse_error(*c, "Error parsing list: components must be seperated by ','.\n"); return false; }
    parse_blank(c);
...
#define declare_parse_argument_(type) \
    ...
    { \
        ...
        { \
            ...
            if (!parse_##type(c, v)) { parse_error(*c, "Error parsing argument value: unexpected type after '='.\n"); return false; } \
            return true; \
        } \
        return false; \
    }
...
static bool parse_param_meta(const char **c, fParams *p, int param)
{
    ...
    if (!parse_end_list(c))
    {
        parse_error(*c, "Failed to parse parameter meta list.\n");
        return false;
    }
    return true;
}
