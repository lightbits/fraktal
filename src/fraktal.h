...
#if 0
// Preliminary API
typedef int ArrayAccess;
typedef int SamplerWrap;
typedef int SamplerFilter;
enum ArrayAccess_   { ARRAY_READ_ONLY=0,       ARRAY_READ_WRITE };
enum SamplerWrap_   { SAMPLER_CLAMP_TO_EDGE=0, SAMPLER_REPEAT };
enum SamplerFilter_ { SAMPLER_LINEAR=0,        SAMPLER_NEAREST };

// These functions let you create a kernel by linking together multiple files.
void *fraktal_link_create();
void fraktal_link_add_file(void *linker, const char *filename);
void fraktal_link_complete(void *linker);

// Load and compile a kernel directly from a file
void *fraktal_load_kernel_file(const char *filename);

// Load and compile a kernel built using link_create and link_complete.
void *fraktal_load_kernel_link(void *linker);

void fraktal_run_kernel(void *output_array, void *kernel, int dim_x, int dim_y);

void *fraktal_create_array2D_uint8(void *data, int dim_x, int dim_y, int components, ArrayAccess access=ARRAY_READ_ONLY);
void *fraktal_create_array2D_float(void *data, int dim_x, int dim_y, int components, ArrayAccess access=ARRAY_READ_ONLY);
void *fraktal_create_array1D_float(void *data, int dim, int components, ArrayAccess access=ARRAY_READ_ONLY);
void fraktal_zero_array(void *handle);

// Transfer a GPU array into CPU memory. The number of bytes to transfer
// is automatically deduced from the GPU type. Note: gpu_array MUST be
// a valid pointer returned from one of the create_array* functions.
void *fraktal_gpu_array_to_cpu(void *cpu_memory, void *gpu_array);
...
void fraktal_input_array2D(const char *name, void *handle, SamplerWrap wrap, SamplerFilter filter);
void fraktal_input_array1D(const char *name, void *handle, SamplerWrap wrap, SamplerFilter filter);
